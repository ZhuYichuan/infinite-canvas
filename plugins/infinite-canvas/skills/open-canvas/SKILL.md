...
```text
https://canvas.imihoo.com/canvas?mode=new#agentUrl=<Local URL>&agentToken=<Connect token>
```
...
插件在新的 Codex 任务中加载时会自动启动 `npx -y @zhuyichuan/canvas-agent mcp`。这个 MCP 进程负责提供画布工具，不提供网页连接服务；
