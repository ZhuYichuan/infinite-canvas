...
In a new Codex conversation, ask it to open Infinite Canvas, create a canvas, and connect Codex. The plugin starts Canvas Agent, reads its connection URL and token, opens `https://canvas.imihoo.com/`, and creates and connects a canvas. It starts a local frontend only when explicitly requested.
